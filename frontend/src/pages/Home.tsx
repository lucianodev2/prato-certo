    ...
    <div className="max-w-6xl mx-auto px-4 py-8">
      ...
      {!online && (
        <div className="mb-4 bg-red-50 border border-red-200 rounded-xl px-4 py-3 text-red-700 text-sm">
          <strong>Modo offline.</strong> As avaliações serão salvas localmente e sincronizadas quando a conexão voltar.
        </div>
      )}
      {online && pendingCount > 0 && (
        <div className="mb-4 bg-yellow-50 border border-yellow-200 rounded-xl px-4 py-3 text-yellow-700 text-sm">
          <strong>{pendingCount}</strong> avaliação(ões) salva(s) offline aguardando sincronização automática.
        </div>
      )}

      {/* Hero */}
      <div className="rounded-3xl overflow-hidden mb-8 bg-gradient-to-br from-brand-700 via-brand-600 to-brand-500 text-white px-8 py-12">
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          <StatCard label="Avaliações" value={overview.totalEvaluations} color="green" sub="no total" />
          <StatCard label="Nota Média"  value={`${overview.averageRating}★`} color="blue"   sub="de 5 estrelas" />
          <StatCard label="Desperdício" value={`${overview.wastePercentage}%`} color="orange" sub="dos alunos" />
          <StatCard label="Refeições"  value={overview.totalMeals} color="green" sub="cadastradas" />
        </div>
      )}
      ...
      <div className="card mb-8">
        <h2 className="font-bold text-gray-700 mb-4">Fluxo do Sistema</h2>
            ...
            {recent.map(ev => (
              <div key={ev.id} className="flex items-center justify-between py-2 border-b border-gray-100 last:border-0">
                ...
                <div className="flex items-center gap-2 flex-wrap justify-end">
                  ...
                  <span className={`badge text-xs ${ev.liked ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-600'}`}>
                    {ev.liked ? 'Gostou' : 'Não gostou'}
                  </span>
                  {ev.had_waste === 1 && (
                    <span className="badge text-xs bg-orange-100 text-orange-600">Desperdício</span>
                  )}
                </div>
              </div>
            ))}
